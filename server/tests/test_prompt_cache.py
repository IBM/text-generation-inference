...
def integration_test_prompts():
    with patch(
        "text_generation_server.prompt_cache.PREFIX_STORE_PATH",
        Path(os.path.join(INTEGRATION_TESTS_DIR, "prompt_prefixes")),
    ):
        yield
...
def test_single_node_list_add_as_head():
    ...
    node = prompt_cache.PromptCacheNode(
...
def test_multi_node_list_add_as_head():
    """Ensure that we can create a list with a single node correctly."""
    dll = prompt_cache.DoublyLinkedList()
    node1 = prompt_cache.PromptCacheNode(
        (torch.ones((3, 3)), torch.ones((3, 3))),
        prefix_id="1",
    )
    node2 = prompt_cache.PromptCacheNode(
        (torch.ones((3, 3)), torch.ones((3, 3))),
        prefix_id="2",
    )
    node3 = prompt_cache.PromptCacheNode(
        (torch.ones((3, 3)), torch.ones((3, 3))),
        prefix_id="3",
    )
...
def test_prompt_cache_node_tensor():
    ...
    if initial_memory is not None:
        assert (
            torch.cuda.memory_allocated() - initial_memory == expected_memory_allocation
        )
...
@patch("text_generation_server.prompt_cache.PrefixCache._load_embedding_tensors")
def test_get_prompt_cache_with_eviction(
    mock_load_tensors,
    mock_get_prompt_size,
    temp_prompt_cache,
):
    """Ensure that if we need to make space, we evicted the least recently used tensor."""
    mock_load_tensors.return_value = torch.ones((3, 3))
    mock_get_prompt_size.return_value = (
        None,
        (prompt_cache.PROMPT_CACHE_SIZE_MB / 2) - 1,
    )
...
@patch("text_generation_server.prompt_cache.PromptCacheNode._get_prompt_stats")
@patch("text_generation_server.prompt_cache.PrefixCache._load_embedding_tensors")
def test_get_prompt_cache_tensor_too_large(
    mock_load_tensors,
    mock_get_prompt_size,
    temp_prompt_cache,
):
...
@patch("torch.load")
@patch("pathlib.Path.is_file")
def test_prompt_too_many_virtual_tokens(
    mock_is_file,
    mock_torch_load,
    temp_prompt_cache,
):
...
@patch("torch.load")
@patch("pathlib.Path.is_file")
def test_prompt_with_infinite_after_conversion(
    mock_is_file,
    mock_torch_load,
    temp_prompt_cache,
):
...
def test_prompt_cache_decoder_only_load(
    temp_prompt_cache,
    tiny_starcoder_decoder_prompt,
):
    ...
    decoder_pt_path = os.path.join(
        prompt_cache.PREFIX_STORE_PATH,
        tiny_starcoder_decoder_prompt,
        "decoder.pt",
    )
...
def test_prompt_cache_peft_decoder_load(
    temp_prompt_cache,
    tiny_raw_llama_peft_adapter_prompt,
):
...
def test_prompt_cache_safetensors_load(
    temp_prompt_cache,

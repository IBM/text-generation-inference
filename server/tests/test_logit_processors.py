...
FULL_SCORES = torch.softmax(
    torch.rand((BATCH_SIZE, VOCAB_DIM), dtype=torch.float32),
    dim=-1,
)
...
def test_alignment_repetition_penalty_logits_processor():
    ...
    s_warped = []
    for penalty, logits, ids in zip(penalties, FULL_SCORES, INPUT_IDS, strict=False):
...
def test_alignment_temperature_logits_processor():
    ...
    s_warped = []
    for temp, logits in zip(temperatures, FULL_SCORES, strict=False):
...
def test_alignment_top_k_logits_processor(top_k):
    ...
    s_warped = []
    for k, logits in zip(top_k, FULL_SCORES, strict=False):
...
def test_alignment_top_p_logits_processor():
    ...
    s_warped = []
    for p, logits in zip(top_p, FULL_SCORES, strict=False):
...
def test_alignment_typical_logits_processor():
    ...
    s_warped = []
    for mass, logits in zip(masses, FULL_SCORES, strict=False):

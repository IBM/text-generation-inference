...
class InferenceEngine(BaseInferenceEngine):
    def __init__(
        ...
        if quantize == "bitsandbytes":
            ...
            quantize == "gptq" and model_config.quantization_config.get("bits", 4) == 4
        ):
            ...
            raise ValueError(
                f"{quantize} quantization not supported by hf_transformers engine",
            )
        else:
        ...
        with self.device:
            self.model = (
                model_class.from_pretrained(**kwargs).requires_grad_(False).eval()
            )

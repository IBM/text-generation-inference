...
class InferenceEngine(BaseInferenceEngine):
    def __init__(
        ...
        if sharded:
            ...
            raise ValueError(
                "tgis_native engine must be used with FLASH_ATTENTION, num_shards > 1 and/or BLOOM or T5",
            )
        ...
        if model_type == "bloom":
            ...
            if sharded and self._config.alibi:
                raise NotImplementedError(
                    "TP is not supported for Falcon models using alibi",
                )
            aliases = {
        ...
        if not filenames:
            raise ValueError(
                "No safetensors weights found - required for tgis_native engine",
            )

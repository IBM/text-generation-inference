...
from text_generation_server.models.model import (
    CUDA_PAD_TO_MULT_OF_8,
    PT2_COMPILE,
    Model,
)
...
class Seq2SeqLMBatch(Batch):
        ...
        if decoder_prefix_ids:
            ...
            for i, dp in decoder_prefix_ids.items():
                ...
                decoder_attention_mask[
                    i,
                    -length - padding_right_offset : -padding_right_offset,
                ] = 1

            # These do not actually get passed to the model
            decoder_input_ids = input_ids.new_zeros(
                (batch_size, max_decoder_input_length),
            )
            ...
            if PT2_COMPILE:
                decoder_attention_mask = attention_mask.new_zeros(
                    batch_size,
                    max_decoder_input_length + padding_right_offset,
                )
    ...
    def concatenate(cls, batches: list["Seq2SeqLMBatch"]) -> "Seq2SeqLMBatch":
        ...
        for i, batch in enumerate(batches):
            ...
            if all_decoder_input_ids_tensor is None:
                all_decoder_input_ids_tensor = batches[
                    0
                ].all_decoder_input_ids_tensor.new_full(
    ...
    def prune(
        ...
        batch.decoder_attention_mask = (
            batch.decoder_attention_mask[
                ...
                -(batch.padding_right_offset + batch.max_decoder_input_length) : (
                    batch.decoder_attention_mask.shape[1] - batch.padding_right_offset
                )
        ...
        batch.input_ids = (
            batch.input_ids[keep_indices] if batch.input_ids is not None else None
        )
        ...
        batch.all_decoder_input_ids_tensor = batch.all_decoder_input_ids_tensor[
            keep_indices,
            -(batch.padding_right_offset + batch.max_decoder_input_length) : (
                batch.all_decoder_input_ids_tensor.shape[1] - batch.padding_right_offset
            )
            + new_padding_right_offset,
        ]
...
class Seq2SeqLM(Model):
    def __init__(
        ...
        self.batch_type = (
            Seq2SeqLMBatch if keys_head_dim_last else KeysDimTransposedSeq2SeqLMBatch
        )
    ...
    def generate_token(
        ...
        (
            ...
            input_ids=batch.all_decoder_input_ids_tensor[
                :,
                : -batch.padding_right_offset,
            ],
            scores=logits[:, -1, :],
...
class KeysDimTransposedSeq2SeqLMBatch(Seq2SeqLMBatch):
    ...
    def from_pb(
        ...
        **kwargs,
        ...
        batch, errors = super(KeysDimTransposedSeq2SeqLMBatch, cls).from_pb(
            *args,
            **kwargs,
        )

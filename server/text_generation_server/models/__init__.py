...
from text_generation_server.models.model import PT2_COMPILE, Model
from text_generation_server.models.seq2seq_lm import Seq2SeqLM
from text_generation_server.utils.dist import get_torch_dtype, print_rank_n
from text_generation_server.utils.hub import TRUST_REMOTE_CODE, get_model_path
...
def get_model(
    ...
    supports_causal_lm = (
        model_type in modeling_auto.MODEL_FOR_CAUSAL_LM_MAPPING_NAMES
        or type(model_config) in AutoModelForCausalLM._model_mapping
        or (
            hasattr(model_config, "auto_map")
            and "AutoModelForCausalLM" in model_config.auto_map
        )
    )
    supports_seq2seq_lm = (
        ...
        or (
            hasattr(model_config, "auto_map")
            and "AutoModelForSeq2SeqLM" in model_config.auto_map
        )
    )
    ...
    if deployment_framework != "tgis_native" and (
        model_type == "bloom" or model_type == "t5"
    ):
        print_rank_n(

...
class PromptCacheNode:
    ...
    def __init__(
        self,
        prompt: torch.Tensor | tuple[torch.Tensor, torch.Tensor],
        prefix_id: str,
        ...
        self.prompt = prompt
        (
            self.prompt_virtual_tokens,
            self.prompt_size_mb,
        ) = PromptCacheNode._get_prompt_stats(prompt)
        self.next = next
        self.prev = prev

    @staticmethod
    def _get_prompt_stats(
        prompt: torch.Tensor | tuple[torch.Tensor, torch.Tensor],
    ) -> tuple[int, int]:
        ...
        # a tensor, take the size of all contained tensor objects.
            ...
            return tuple(
                sum(x)
                for x in zip(
                    *map(PromptCacheNode._get_prompt_stats, prompt),
                    strict=False,
                )
            )
...
class PrefixCache:
    ...
    def __init__(
        ...
        return_zero: bool | None,
        decoder_start_tok_embedding: torch.Tensor,
    ):
        self.max_length = max_length
        self.embed_size = (
            decoder_start_tok_embedding.shape[1]
            if decoder_start_tok_embedding is not None
            else None
        )
        ...
        self.zero = (
            torch.zeros((1,), dtype=dtype, device=device) if return_zero else None
        )
    ...
    def get(self, prefix_id: str) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
    ...
    def _reject_bad_prefix_ids(prefix_id: str) -> None:
        ...
        if not VALID_PREFIX_ID_PATTERN.fullmatch(prefix_id):
            raise Exception(
                f"Invalid prefix id {prefix_id}, must contain only alphanumeric, _ and - and /",
            )
        prefix_dir_path = PREFIX_STORE_PATH / prefix_id
        # Check for path traversal
        if not os.path.normpath(prefix_dir_path).startswith(
            str(PREFIX_STORE_PATH) + "/",
        ):
    ...
    def _is_peft_prefix(prefix_id):
        ...
        if not os.path.isdir(prefix_dir_path):
            return False
        return "adapter_model" in [
            os.path.splitext(f)[0] for f in os.listdir(prefix_dir_path)
        ]

    def _load_embedding_tensors_peft(
        self,
        prefix_id: str,
        ...
        if self.is_encoder_decoder:
            raise Exception(
                "encoder-decoder architectures not supported for peft models",
            )
        ...
        decoder_data_dict = self._load_torch_file(
            prefix_id,
        ...
        if self.is_encoder_decoder:
            if decoder_prefix is not None:
                ...
                decoder_prefix = torch.cat(
                    (decoder_prefix, self.decoder_start_tok_embedding),
                )
    ...
    def _process_prefix_tensor(
        ...
        if not torch.is_tensor(prefix) or len(prefix.shape) != 2:
            raise Exception("Invalid prefix embedding tensor")
        ...
        if self.is_encoder_decoder and prefix.shape[0] % 2 != 0:
            raise Exception("Encoder/decoder prefix tensor must be of even length")
        ...
        if not converted_prefix.isfinite().all():
            ...
            if not prefix.isfinite().all():
                raise Exception("Prefix contains non-finite elements")
            raise Exception(
                f"Prefix contains non-finite elements after conversion from {prefix.dtype} to {dtype}",
            )
    ...
    def _add_prefix_id_to_cache(
        self,
        prefix_id: str,
        prefix: torch.Tensor | tuple[torch.Tensor, torch.Tensor],
    ) -> PromptCacheNode:
        ...
        with self.requires_lock:
            ...
            if new_cache_node.prompt_size_mb > PROMPT_CACHE_SIZE_MB:
                raise ValueError(
                    f"Prefix ID object {prefix_id} exceeds the allowed cache size",
                )
        ...
        if del_tensors:
            logger.info(
                f"Deleted prefixes {list(del_tensors.keys())} from the prompt cache",
            )

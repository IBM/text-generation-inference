...
def pt2_compile_warmup(
    ...
    def __safe_eval_shape(batch_size: int, input_length: int, num_new_tokens: int):
        try:
            if batch_size == 0 or input_length == 0 or num_new_tokens == 0:
                ...
                print(
                    f">> skipping __eval_shape({batch_size}, {input_length}, {num_new_tokens}) due to zero argument",
                )
                return
    ...
    for batch_size in [1, max_batch_size]:
        ...
        max_input_len = memory_scaling_model.max_input_len_for_prefill(
            batch_size,
    ...
    for batch_size in [1, max_batch_size]:
        ...
        max_input_len = memory_scaling_model.max_input_len_for_nt(
            batch_size,
            2,
        ...
        max_output_len = memory_scaling_model.max_output_len_for_nt(
            batch_size,
            1,
            max_new_tokens,
        )
        ...
        if max_output_len == max_new_tokens:
            ...
            max_input_len = memory_scaling_model.max_input_len_for_nt(
                batch_size,
                mid_output_len,
                max_sequence_length - mid_output_len,
            )
            __safe_eval_shape(
                batch_size=batch_size,
                input_length=max_input_len,
    ...
    for i in range(n_samples):
        ...
        try:
            ...
            max_output_len_ = memory_scaling_model.max_output_len_for_nt(
                batch_size,
                input_len,
                max_output_len,
            )
        ...
        if verbose:
            ...
            print(
                ">> n_samples: %3d, n_new_compiles: %3d"
                % (i + 1, n_kernels - n_kernels_init),
            )
